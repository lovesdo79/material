...
(function($) {
	...
	// }
	// } else {
	// var pos = slider.height() -
	// (value-opts.min)/(opts.max-opts.min)*slider.height();
	// if (opts.reversed){
	...
	// } else {
	// var value = opts.min +
	// (opts.max-opts.min)*((slider.height()-pos)/slider.height());
	// }
	...
	function pos2value(target, pos) {
		...
		var slider = state.slider;
		var size = opts.mode == 'h' ? slider.width() : slider.height();
		var value = opts.converter.toValue.call(target, opts.mode == 'h'
						? (opts.reversed ? (size - pos) : pos)
						: (size - pos), size);
		return value.toFixed(0);
		// var value = opts.converter.toValue.call(target,
		// opts.mode=='h'?pos:(size-pos), size);
		// return opts.reversed ? opts.max - value.toFixed(0) :
		// value.toFixed(0);
	}
