var detailview = $.extend({}, $.fn.datagrid.defaults.view, {
	render : function(target, container, frozen) {
		...
		var table = [];
		table
				.push('<table class="datagrid-btable" cellspacing="0" cellpadding="0" border="0" style="width:100px"><tbody>');
	...
	onBeforeRender : function(target) {
		...
		if (!hasExpander) {
			...
			var td = $('<td rowspan="'
					...
					+ '"><div class="datagrid-header-expander" style="width:100px;text-align: center;">变更内容</div></td>');
			if ($('tr', t).length == 0) {
		...
		return jq.each(function() {
			...
			function createGrid(target, conf, prow) {
				...
				queryParams[conf.options.foreignField] = prow
						? prow[conf.options.foreignField]
						: undefined;
