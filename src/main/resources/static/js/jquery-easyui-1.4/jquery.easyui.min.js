...
(function($) {
	...
	function _af(_b0, _b1) {
		...
		// // 去除合计行统计
		// if
		// ($("#gridList").datagrid('getPager').data("pagination").options.total
		// > 0) {
		// var myTotal = _b3.total;
		// if (myTotal > 0 && myTotal % _b3.pageSize != 0) {
		// myTotal = _b3.total - 1;
		// }
		// }
		// } catch (e) {
...
(function($) {
	...
	$.messager = {
		...
		// 添加message.alert在固定时间关闭;isAutoClose:是否自动关闭; duration:持续时间
		alert : function(_296, msg, icon, fn, isAutoClose, duration) {
			// 判断是否点击已经点击OK
			var isClickOK = false;
	...
	$.fn.validatebox.defaults = {
		...
		rules : {
			...
			specialChar : {
				...
				message : "Please do not input special char."
			},
			// 最大值验证
			maxValue : {
				validator : function(_44e, _44f) {
					var data = parseFloat(parseFloat(_44e).toFixed(2));
					var max = parseFloat(parseFloat(_44f[0]).toFixed(2));
					if (data > max) {
						return false;
					} else {
						return true;
				...
				message : "The maximum allowed no more than {0}."
			},
			url : {
...
(function($) {
	...
	$.fn.numberbox.defaults = $.extend({}, $.fn.textbox.defaults, {
		...
		parser : function(s) {
			...
			var number = null;
			var dotIndex = float.indexOf(".");
			if (dotIndex == -1) {
				Number(float).toFixed(opts.precision);
			} else {
				var head = float.substring(0, dotIndex + 4);
				var lastNumber = head.substring(dotIndex + 3, dotIndex + 4);
				if (Number(lastNumber) == 5) {
					if (head.indexOf("-") == -1) {
						float = Number(head) + 0.001;
					} else {
						float = Number(head) - 0.001;
					}
			...
			var val = parseFloat(float).toFixed(opts.precision);
			// var val = (Math.round(parseFloat(s) *
...
(function($) {
	...
	function _55f(a, o, id) {
		if (typeof o == "string") {
			for (var i = 0, len = a.length; i < len; i++) {
				// Start 2015年8月10日 15:50:22
				// if (a[i][o] == id) {
				if (typeof(a[i]) != 'undefined' && a[i][o] == id) {
					// End
					a.splice(i, 1);
					return;
				}
	...
	function _561(a, o, r) {
		for (var i = 0, len = a.length; i < len; i++) {
			// Start 2015年8月10日 15:50:22
			// if (a[i][o] == r[o]) {
			if (typeof(a[i]) != 'undefined' && a[i][o] == r[o]) {
				// End
				return;
			}
		}
	...
	var _711 = {
		...
		renderRow : function(_721, _722, _723, _724, _725) {
			...
			for (var i = 0; i < _722.length; i++) {
				...
				if (col) {
					// start 支持子属性 edit by xdwang 2015年5月15日 16:47:11
					var _728 = _725[_727];
					// var _728=eval("_725['"+_727.replace(/\./g,"']['")+"']");
					// end 支持子属性 edit by xdwang 2015年5月15日 16:47:11
						...
						t.combo('textbox').bind("input", function(e) {
							...
							_8a8.timer = setTimeout(function() {
										...
										if (_8a8.previousText != q) {
											_8a8.previousText = q;
											t.combo("showPanel");
											opts.keyHandler.query.call(_8a7, q,
													e);
											t.combo("validate");
										}
									}, opts.delay);
...
(function($) {
	...
	function _8e5(_8e6, _8e7) {
		...
		if ($.inArray(_8e7 + "", _8e8) == -1) {
			...
			_8e9(_8e6, _8e8);
			// start 修改combobox只有一个选项时执行onselect事件 2015年8月8日 20:35:02
	...
	function _902(_903) {
		...
		// start 保证combobox只有一个选项时，也能执行onselect 2015年8月8日 20:33:06
		t.combobox("unselect", vv);
		t.combobox("select", vv);
		// end
		if (!opts.multiple) {
...
(function($) {
	function _969(_96a) {
		...
		// 禁止手动输入时间
		$(_96a).combo("textbox").keydown(function() {
					return false;
				});
		if (!_96b.calendar) {
			_96d();
	...
	function _974(_979, _97a, _97b) {
		...
		// 清空
		if (_97a == "null") {
			$(_979).combo("setValue", "").combo("setText", "");
	...
	$.fn.datebox.defaults = $.extend({}, $.fn.combo.defaults, {
				...
				buttons : [{
							...
							text : function(_985) {
								return $(_985).datebox("options").clearText;
							},
