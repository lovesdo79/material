...
var hljs = new function() {
	...
	function langRe(language, value, global) {
		var mode = 'm' + (language.case_insensitive ? 'i' : '')
				+ (global ? 'g' : '');
	...
	function mergeStreams(stream1, stream2, value) {
		...
		function selectStream() {
			if (stream1.length && stream2.length) {
				if (stream1[0].offset != stream2[0].offset)
					...
					return (stream1[0].event == 'start' && stream2[0].event == 'stop')
							? stream2
							: stream1
			} else {
		...
		while (stream1.length || stream2.length) {
			...
			result += escape(value
					.substr(processed, current.offset - processed));
	...
	function highlight(language_name, value) {
		...
		function keywordMatch(mode, match) {
			...
			for (var className in mode.keywordGroups) {
				if (!mode.keywordGroups.hasOwnProperty(className))
					continue;
				var value = mode.keywordGroups[className]
		...
		function processKeywords(buffer, mode) {
			...
			while (match) {
				result += escape(buffer.substr(last_index, match.index
								- last_index));
				var keyword_match = keywordMatch(mode, match);
				if (keyword_match) {
					keyword_count += keyword_match[1];
					result += '<span class="' + keyword_match[0] + '">'
							+ escape(match[0]) + '</span>'
				} else {
			...
			result += escape(buffer.substr(last_index, buffer.length
							- last_index));
			return result
		};
		...
		function startNewMode(mode, lexem) {
			var markup = mode.noMarkup ? '' : '<span class="'
		...
		function processModeInfo(buffer, lexem, end) {
			...
			if (end) {
				result += processBuffer(current_mode.buffer + buffer,
						current_mode);
				return false
			};
		...
		try {
			...
			do {
				...
				var return_lexem = processModeInfo(mode_info[0], mode_info[1],
						mode_info[2]);
			...
			for (var j = 0; j < language.modes.length; j++) {
				...
				if (mode.illegal)
					mode.illegalRe = langRe(language, '^(?:' + mode.illegal
									+ ')');
				language.defaultMode.illegalRe = langRe(language, '^(?:'
	...
	function highlightBlock(block, tabReplace) {
		...
		if (language) {
			...
			for (var key in selected_languages) {
				...
				var relevance = lang_result.keyword_count
						+ lang_result.relevance;
		...
		if (result) {
			...
			container.innerHTML = '<pre><code class="' + class_name + '">'
					+ result + '</code></pre>';
