...
_demoSkin.tmpl = function(data) {
	...
	return html.join('');
			...
			name : 'default',
			about : 'artDialog默认皮肤，简洁，纯CSS设计，无图片，采用css3渐进增强'
		}, {
			name : 'aero',
			...
			name : 'chrome',
			about : 'chrome浏览器(xp)风格'
		}, {
			name : 'opera',
			about : 'opera 11浏览器内置对话框风格'
		}, {
			name : 'simple',
			about : '简单风格，无图片，不显示标题'
		}, {
			name : 'idialog',
			about : '苹果风格，iPad Safari或Mac Safari关闭按钮将在左边显示'
		}, {
			name : 'twitter',
			about : 'twitter风格，无图片'
		}, {
...
$(function() {
			...
			// 点亮导航
			var links = $('#nav')[0].getElementsByTagName("a"), URL = document.URL
					.split('#')[0], last = URL.charAt(URL.length - 1);
